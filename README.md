...
- `.txt`, `.md`, `.csv`, `.tsv`, `.json`, `.xml`, `.log`, `.doc`, `.docx`

For office formats, convert before upload:
- `.xlsx` / `.xls` → export to `.csv`
- `.pdf` / `.pptx` → export to `.txt` or `.md`
- `.docx`/`.doc` upload is supported, but plain-text conversion quality depends on source formatting
